...
def find_sheet_paper(frame, thresh, add_margin=True):
    ...
    stats = functions.find_corners(thresh)
    # First point is center of coordinate system, so ignore it
    # We only want sheet of paper's corners
    corners = stats[1:, :2]
    corners = functions.order_points(corners)
    # Get bird view of sheet of paper
    paper = functions.four_point_transform(frame, corners)
    if add_margin:
...
def find_shape(cell):
    ...
    cell = functions.preprocess_input(cell)
...
def get_board_template(thresh):
    ...
    middle_center = functions.contoured_bbox(thresh)
